...
export function mountReviewWorkbench(getContext){
  ...
  const q=s=>dialog.querySelector(s),player=q('[data-video]');let session=null,selected=null,playback=null,context=null,evaluation=null,lockSha256=null,revision=0;
  ...
  function tick(){if(!session)return;session=tickReview(session,Math.max(now(),session.lastTick),{visible:!document.hidden&&dialog.open});}
  ...
  async function restore(storageKey){const request=++revision;try{
    tick();persist();
    ...
    const restoredEvaluation=restored.status==='finished'&&saved.evaluation?.reference?evaluateReview(restored,saved.evaluation.reference):null;
    if(session&&session.id!==restored.id){tick();localStorage.setItem(`courtvision-review-archive:${session.id}`,JSON.stringify({session,evaluation,lockSha256}));}
  ...
  document.addEventListener('visibilitychange',()=>{tick();playback=null;if(document.hidden)player.pause();persist();});
